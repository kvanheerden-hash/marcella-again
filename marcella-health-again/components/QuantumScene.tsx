...
const MacroscopicWave = () => {
  ...
  return (
    <Torus ref={ref} args={[3, 0.1, 16, 100]} rotation={[Math.PI / 2, 0, 0]}>
      <meshStandardMaterial color="#854DFF" emissive="#854DFF" emissiveIntensity={0.5} transparent opacity={0.6} wireframe />
    </Torus>
  );
}
    ...
    <div className="absolute inset-0 z-0 opacity-60 pointer-events-none">
      <Canvas camera={{ position: [0, 0, 6], fov: 45 }}>
        ...
        <Float speed={1.5} rotationIntensity={0.2} floatIntensity={0.5}>
          <QuantumParticle position={[0, 0, 0]} color="#854DFF" scale={1.2} />
          <MacroscopicWave />
        </Float>
        
        <Float speed={2} rotationIntensity={0.5} floatIntensity={1}>
           <QuantumParticle position={[-3, 1, -2]} color="#061551" scale={0.5} />
  ...
  return (
    <div className="w-full h-full absolute inset-0">
      <Canvas camera={{ position: [0, 0, 4.5], fov: 45 }}>
        ...
        <spotLight position={[5, 5, 5]} angle={0.3} penumbra={1} intensity={2} color="#854DFF" />
        <pointLight position={[-5, -5, -5]} intensity={0.5} />
        <Environment preset="studio" />
        
        <Float rotationIntensity={0.4} floatIntensity={0.2} speed={1}>
          <group rotation={[0, 0, 0]} position={[0, 0.5, 0]}>
            {/* Main Cryostat Structure (Purple Chandelier) */}
            
            {/* Top Plate */}
            <Cylinder args={[1.2, 1.2, 0.1, 64]} position={[0, 1, 0]}>
              <meshStandardMaterial color="#854DFF" metalness={1} roughness={0.15} />
            </Cylinder>
            ...
            {/* Coils/Wires - Copper colored (kept as accent) */}
            ...
            <Box args={[0.2, 0.05, 0.2]} position={[0, -0.7, 0]}>
                <meshStandardMaterial color="#061551" metalness={0.9} roughness={0.1} />
            </Box>
          </group>
        </Float>
      </Canvas>
